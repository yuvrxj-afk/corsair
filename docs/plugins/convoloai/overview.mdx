---
title: Overview
description: "ConvoloAi plugin for Corsair"
---

Use **ConvoloAi** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 31 typed API operations
- 4 synced entities (`agents`, `calls`, `leads`, `widgets`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/convoloai
```
```bash yarn
yarn add corsair @corsair-dev/convoloai
```
```bash pnpm
pnpm install corsair @corsair-dev/convoloai
```
```bash bun
bun add corsair @corsair-dev/convoloai
```
</CodeGroup>
...
import { convoloai } from '@corsair-dev/convoloai';
...
await tenant.convoloai.api.agent.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.convoloai.api.agent.create({});
```


See the full list on the [API](/plugins/convoloai/api) page.

## Query synced data

Synced entities support `tenant.convoloai.db.<entity>.search()` and `.list()`: `agents`, `calls`, `leads`, `widgets`.

See [Database](/plugins/convoloai/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/convoloai/api">
    Every `convoloai.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/convoloai/database">
