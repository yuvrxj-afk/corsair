...
Use **Gladia** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/gladia
```
```bash yarn
yarn add corsair @corsair-dev/gladia
```
```bash pnpm
pnpm install corsair @corsair-dev/gladia
```
```bash bun
bun add corsair @corsair-dev/gladia
```
</CodeGroup>
...
import { gladia } from '@corsair-dev/gladia';
...
await tenant.gladia.api.live.getTranscriptionResult({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.gladia.api.live.deleteSession({});
```


See the full list on the [API](/plugins/gladia/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/gladia/api">
    Every `gladia.api.*` operation with input and output types.
  </Card>
