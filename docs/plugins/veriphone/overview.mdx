...
Use **Veriphone** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/veriphone
```
```bash yarn
yarn add corsair @corsair-dev/veriphone
```
```bash pnpm
pnpm install corsair @corsair-dev/veriphone
```
```bash bun
bun add corsair @corsair-dev/veriphone
```
</CodeGroup>
...
import { veriphone } from '@corsair-dev/veriphone';
...
await tenant.veriphone.api.coverage({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.veriphone.api.verifyPhoneNumber({});
```


See the full list on the [API](/plugins/veriphone/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/veriphone/api">
    Every `veriphone.api.*` operation with input and output types.
  </Card>
