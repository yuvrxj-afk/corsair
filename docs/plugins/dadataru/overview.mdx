---
title: Overview
description: "Dadataru plugin for Corsair"
---

Use **Dadataru** through Corsair: one client, typed API calls.

**What you get:**

- 63 typed API operations
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/dadataru
```
```bash yarn
yarn add corsair @corsair-dev/dadataru
```
```bash pnpm
pnpm install corsair @corsair-dev/dadataru
```
```bash bun
bun add corsair @corsair-dev/dadataru
```
</CodeGroup>
...
import { dadataru } from '@corsair-dev/dadataru';
...
await tenant.dadataru.api.clean.address({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/dadataru/api) reference._


See the full list on the [API](/plugins/dadataru/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/dadataru/api">
    Every `dadataru.api.*` operation with input and output types.
  </Card>
