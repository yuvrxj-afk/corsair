---
title: Overview
description: "Flexisign plugin for Corsair"
---

Use **Flexisign** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/flexisign
```
```bash yarn
yarn add corsair @corsair-dev/flexisign
```
```bash pnpm
pnpm install corsair @corsair-dev/flexisign
```
```bash bun
bun add corsair @corsair-dev/flexisign
```
</CodeGroup>
...
await tenant.flexisign.api.list.templates({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/flexisign/api) reference._


See the full list on the [API](/plugins/flexisign/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/flexisign/api">
    Every `flexisign.api.*` operation with input and output types.
  </Card>
