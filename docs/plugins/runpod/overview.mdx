---
title: Overview
description: "Runpod plugin for Corsair"
---

Use **Runpod** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 14 typed API operations
- 7 synced entities (`clusters`, `endpoints`, `pods`, `registryAuths`, `secrets`, `templates`, and 1 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/runpod
```
```bash yarn
yarn add corsair @corsair-dev/runpod
```
```bash pnpm
pnpm install corsair @corsair-dev/runpod
```
```bash bun
bun add corsair @corsair-dev/runpod
```
</CodeGroup>
...
import { runpod } from '@corsair-dev/runpod';
...
await tenant.runpod.api.account.myself({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.runpod.api.account.updateSettings({});
```


See the full list on the [API](/plugins/runpod/api) page.

## Query synced data

Synced entities support `tenant.runpod.db.<entity>.search()` and `.list()`: `clusters`, `endpoints`, `pods`, `registryAuths`, `secrets`, `templates`, `users`.

See [Database](/plugins/runpod/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/runpod/api">
    Every `runpod.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/runpod/database">
