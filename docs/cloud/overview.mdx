...
const corsair = createCorsairCloud({ apiKey: process.env.CORSAIR_CLOUD_KEY! });
...
```bash curl
# The key carries the address; for curl, put the runtime URL from the dashboard here.
...
- **From a browser (React).** Browser code must never see the key. Stand up a
  route in *your own* app that injects the bearer server-side; `corsair/cloud-proxy`
...
`createCorsairCloud` takes no plugin list — the plugins live on the runtime — so
